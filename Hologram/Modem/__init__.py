...
class Modem:
    ...
    def isConnected(self):
        return self._ppp.isConnected()

    def connect(self, timeout):
        if self._ppp is None:
            self._ppp = PPP(device_name=self.device_name,
                             all_attached_device_names=self.__detect_all_serial_ports(),
                             baud_rate=self.baud_rate,
                             chatscript_file=self.chatscript_file)
        return self._ppp.connect(timeout = timeout)


    def disconnect(self):

        if self._ppp is not None:
            res = self._ppp.disconnect()
            self._ppp = None
            return res
        else:
            try:
    ...
    def localIPAddress(self):
        if self._ppp:
            return self._ppp.localIPAddress
        else:
            return None
    ...
    def remoteIPAddress(self):
        if self._ppp:
            return self._ppp.remoteIPAddress
        else:
            return None
